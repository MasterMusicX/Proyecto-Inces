...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 animate-fade-in-up">
    ...
    <a href="<?php echo e(route('student.courses.show', $course)); ?>" class="inline-flex items-center text-sm font-bold text-gray-500 dark:text-slate-400 hover:text-blue-800 dark:hover:text-blue-400 transition-colors mb-4">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-slate-700/50 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight"><?php echo e($course->title); ?></h1>
                <p class="text-sm font-medium text-gray-500 dark:text-slate-400 mt-1">Instructor: <span class="font-bold text-gray-700 dark:text-slate-300"><?php echo e($course->instructor->name); ?></span></p>
            </div>
        </div>
        
        <a href="<?php echo e(route('student.chatbot')); ?>" class="shrink-0 px-5 py-2.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/50 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors shadow-sm w-full sm:w-auto">
            🤖 Asistente IA
        </a>
    </div>
        ...
        <div class="bg-white dark:bg-[#1e293b] rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700/50 overflow-hidden" x-data="{ open: true }">
            ...
            <button @click="open = !open" class="w-full flex items-center justify-between p-6 hover:bg-gray-50 dark:hover:bg-[#0f172a] transition-colors focus:outline-none">
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-left">
                        <h3 class="font-extrabold text-gray-900 dark:text-white text-base"><?php echo e($module->title); ?></h3>
                        <span class="text-xs font-bold text-gray-500 dark:text-slate-400 mt-1 block"><?php echo e($module->resources->count()); ?> recursos</span>
                    </div>
                </div>
            ...
            <div x-show="open" x-collapse x-cloak class="border-t border-gray-100 dark:border-slate-700/50 bg-gray-50/50 dark:bg-[#0f172a]/30">
                <div class="p-2">
                    <?php $__empty_2 = true; $__currentLoopData = $module->resources->where('is_published', true); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $resource): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                        <a href="<?php echo e(route('student.resources.show', $resource)); ?>" class="flex items-center gap-4 p-4 hover:bg-white dark:hover:bg-[#1e293b] rounded-2xl transition-all border border-transparent hover:border-gray-200 dark:hover:border-slate-600 hover:shadow-sm group my-1">
                            
                            <span class="text-2xl flex-shrink-0 group-hover:scale-110 transition-transform"><?php echo e($resource->type_icon); ?></span>
                            
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-sm text-gray-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"><?php echo e($resource->title); ?></p>
                                <?php if($resource->description): ?>
                                    <p class="text-xs text-gray-500 dark:text-slate-400 truncate mt-0.5"><?php echo e($resource->description); ?></p>
                                <?php endif; ?>
                            </div>

                            <div class="flex items-center gap-4 flex-shrink-0">
                                <div class="text-right hidden sm:block">
                                    <span class="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block"><?php echo e($resource->type); ?></span>
                                    <?php if($resource->file_size): ?>
                                        <span class="text-[10px] font-bold text-gray-400"><?php echo e($resource->file_size_human); ?></span>
                                    <?php endif; ?>
                                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/inces/Escritorio/Proyecto Inces/resources/views/student/courses/learn.blade.php ENDPATH**/ ?>