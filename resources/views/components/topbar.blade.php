                    ...
                    @if(auth()->check() && auth()->user()->notifications->count() > 0)
                        @foreach(auth()->user()->notifications->take(5) as $notification)
                            <a href="{{ route('notifications.markAsRead', $notification->id) }}" 
                               class="block px-5 py-4 border-b border-gray-50 dark:border-slate-700/30 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors {{ $notification->read_at ? 'opacity-60' : 'bg-blue-50/30 dark:bg-blue-900/10' }}">
                                <div class="flex items-start gap-3">
                                    <div class="text-2xl shrink-0">{{ $notification->data['icon'] ?? '📌' }}</div>
                                    <div>
