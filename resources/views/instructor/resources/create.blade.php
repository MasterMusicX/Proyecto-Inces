...
<div class="max-w-3xl mx-auto py-6">
    ...
    <div class="mb-6 flex items-center gap-3">
        <a href="{{ route('instructor.courses.resources.index', $course) }}" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white flex items-center gap-1 text-sm font-medium transition-colors">
            &larr; Volver
        </a>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden p-6 sm:p-8">
        ...
        {{-- Fíjate que en x-data guardamos el tipo y también el nombre del archivo para hacerlo reactivo --}}
        <form method="POST" action="{{ route('instructor.courses.resources.store', $course) }}"
            enctype="multipart/form-data" class="space-y-6" 
            x-data="{ type: '{{ old('type', 'pdf') }}', fileName: 'Sin archivos seleccionados' }">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Nombre del recurso" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Descripción</label>
                <textarea name="description" rows="3" placeholder="Descripción opcional..." class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('description') }}</textarea>
            </div>
            ...
            <div x-show="type === 'url'" x-transition>
                ...
                <input type="url" name="external_url" value="{{ old('external_url') }}" placeholder="https://..." class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>
            ...
            <div class="flex items-center gap-3 pt-6 mt-6 border-t border-gray-100 dark:border-gray-700">
                ...
                <a href="{{ route('instructor.courses.resources.index', $course) }}" class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 px-6 py-2.5 rounded-lg text-sm font-semibold shadow-sm transition-colors">
                    Cancelar
                </a>
            </div>
